...
# Read a Firebase user's custom claims (role:researcher, admin,
# programmeAdmin) against a deployed env. Read-only. Claims are granted
# PER ENVIRONMENT with no sync between them, which is why "it works on
# test but not prod" is usually a missing grant, not a seeding problem —
# see scripts/check-user-role.sh's header for the incident this closes.
...
# What is ACTUALLY running where. STEP 0 of any promotion: the version to
# promote comes from here, never from a doc or a runbook example — both drift,
...
check-literature-isolation: ## Verify the literature RAG corpus is unreachable from student turns (CI-gated)
	@bash scripts/check-literature-corpus-isolation.sh
...
# here — and the next engagement forks from a template that has none of them.
# Upstream is platform-source, NOT ai-protocol-platform: the public template is
# regenerated by force-push, so a PR against it dies at the next refresh.
# See .template-fork-target and docs/upstream-feedback.md.
# 1.1.101. The SSE filter redacts unless a tool is declared renderable, so any
# tool the client parses BY NAME must be allow-listed or its card silently stops
# rendering for students. mark_checklist_item was missed and caught by hand on
...
upstream-reconcile: ## Full divergence report against the template (every shared path)
	@bash scripts/check-upstream-routing.sh --all

port-up: ## Copy platform improvements up to the template (dry run; GO=1 to push + open a PR)
	@bash scripts/port-up.sh "$(RANGE)" $(if $(GO),GO=1,)
...
help:
	...
	@echo "make port-up            — copy platform improvements up to platform-source (dry run; GO=1 to push + PR)"
	@echo
