...
// artefacts (non-agent-summoned: student clicks a button to open the
// sim, not the agent invoking a tool).
//
...
// have neither (the student summons them, not the agent). This component
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data.method === INITIALIZE && data.id !== undefined && data.id !== null) {
        ...
        const result: McpUiInitializeResult = {
          ...
          capabilities: {},
          serverInfo: { name: "aipla-host", version: "0.1.0" },
          hostContext: {
      ...
      // Anything else: log in dev, ignore in prod. Future artefact
      // capabilities will widen this match list.
      if (process.env.NODE_ENV !== "production") {
        // eslint-disable-next-line no-console
