...
| **Promote preserves env vars, so prod freezes at its first-deploy config** | a per-env value corrected in `cloudbuild.yaml` never reaches prod — prod is reached only by `make promote`, and `gcloud run services update` keeps existing env vars | `cloudbuild.promote.yaml` re-stamps every per-env var (`APP_VERSION`, `MCP_WIDGET_DOMAIN`) and its `guard-version` step fails the build if one is empty | **partly enforced** — the guard covers today's vars; adding a new `--set-env-vars` in `cloudbuild.yaml` without a promote twin re-opens it. `MCP_WIDGET_DOMAIN` sat on DEV's origin in prod from env-cut to 2026-08-04 |
