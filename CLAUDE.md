...
- **`guide-maintenance`** — keep the user-facing how-to guides in sync with the product. Since 1.1.116 they are **app content** (`frontend/content/guides/<slug>.md` → `/guides/<slug>`), not Quarto documents: no render step, no LaTeX, and the old `/guides/<slug>.html` URLs redirect. Covers screenshot-capture on deployed dev, printing the PDFs from the pages (`make guides-pdf`), seeding the in-product corpus + onboarding tutors, and the staleness check. Load when guides go stale or a documented UI surface changes.
...
| **Client-rendered tool result not allow-listed** | a student's card (checklist mark, checkpoint) silently stops rendering — no error, no log line, because the SSE filter replaced the result with the redaction sentinel | anything the client parses by `tc.name` must be in `_CLIENT_RENDER_TOOLS`; `scripts/check-stream-render-allowlist.sh` (`make check-stream-allowlist`) | **enforced** (CI `local-mode-safety`, 1.1.101 2026-09-08) — inverting the filter's default to deny-unless-declared turned a pre-existing accident into a regression: `mark_checklist_item` is outside `TOOL_REGISTRY`, so the OLD rule passed it as an "unknown name", the same loophole that let teacher-authored tools through. Caught by hand between test and prod |
...
| **User-facing text hardcoded inline in JSX / prompt prose / a sim** | a translator cannot reach it without a code change; an English activity shows Danish buttons; the deferred i18n grows by every string written | **language is data, text is content, neither is code** — new UI copy goes in a `const copy = {…}` object at the top of the component (or `t()` once the message layer reaches that surface), never inline in JSX; every new content surface ships with a locale axis (`<slug>.md` + `<slug>.da.md`, the guides' shape); language resolves from `activity.language` / `teacher_prefs.locale`, never `navigator.language`; prompt prose never bakes a language in (the directive comes from config); a sim carries its strings in one object. Full rule: [1.1.108 content-localisation](docs/design/aipla/v1.1.0-feedback/content-localisation.md) M4 | **manual** (rule in force 2026-09-11; `make check-i18n` ships with 1.1.108 M0) — ~213 Danish literals sit inline today because nothing said not to. SH's first-contact reaction on prod was *the Danish is confusing*, and the ~2–3d i18n estimate from August has held only because two people wrote every string; that ends when AD starts |
...
| **A `.gitignore` rule swallows a source directory** | a route ships a nav link to a page that was never committed — and ten tests stop being counted. `git add` refuses it on stderr among other output, **`git commit` succeeds**, `git status` looks clean, and the build stays green because a missing route is not a compile error | anchor directory rules to the repo root (`/logs/`, not `logs/`); `scripts/check-app-routes-tracked.sh` (`make check-routes-tracked`) fails on any app-router page/layout/route/test that git ignores, or (in CI) that is present but untracked | **enforced** (CI `local-mode-safety`, 1.1.109 2026-09-11) — an unanchored `logs/`, meant for log output, matched `frontend/src/app/teacher/research/logs/`. Git applies such a pattern at EVERY depth. The rule was protecting nothing: the only non-vendored match in the tree was that route. ⚠️ **The class is still live** — `dist/`, `build/`, `coverage/` and `htmlcov/` are all still unanchored, and a route by any of those names vanishes identically; they are not anchored speculatively because nested build output may legitimately rely on the loose match, so the guard checks the *outcome* rather than enumerating causes |
| **A second RAG corpus reachable from the wrong audience** | a tutor quotes a copyrighted journal paper at a 16-year-old; or the tutor's prompt starts differing turn to turn, so it can no longer be checked against the paper it claims to implement | AIPLA has TWO corpora with opposite audiences — **curriculum** (student-reachable by design, scoped per activity) and **literature** (researcher authoring only). `db/literature_corpus.py` exposes **no tool builder**, so there is nothing for an agent to be handed; `scripts/check-literature-corpus-isolation.sh` (`make check-literature-isolation`) fails on an import from the agent path, or on that module growing a tool builder | **enforced** (CI `local-mode-safety`, 1.1.110 2026-09-11) — the separation is structural rather than a policy anyone has to remember. ⚠️ Grounding the tutor in the literature would destroy the reviewability the framework layer exists for: retrieval makes the prompt differ per turn, and "hold the prompt against the paper" needs it to be the same thing every time |
| **A resolver ships with one consumer** | the abstraction is correct, its unit tests pass, and the feature is a no-op — a teacher picks a tutor and the student sees the old one. The *bundling* claim is the tell: four axes promised, one wired | before calling a bundling abstraction shipped, name every read site the bundle claims to own and assert each END-TO-END; `test_class_tutor_reaches_the_student.py` does it per axis | **enforced for the tutor bundle** (1.1.112 2026-09-14) — 1.1.91's `resolve_teaching` honoured the class tutor and `test_tutor_resolution.py` proved it in 100+ green lines, but its ONLY consumer was the chat-log stamp + the framework preamble. Name/avatar, voice and tone went on reading `class.persona`, which `update_class_tutor` never writes — and since no base tutor carries a framework *by design*, picking any tutor a teacher can select changed **nothing**. `d05500d2` had already removed the old persona panel, so prod had no working control at all for four days. ⚠️ A test seeding only a tutor PASSES against this bug: the persona chain falls through to the global default, which is Sofie. The stored fields must CONFLICT to witness it. Same shape as the dropped trust card and the missing promote twin — a two-ended wiring where one end is invisible from the end you are looking at |
| **A whole stack ships with the control unmounted** | the endpoint works, the API client has a passing test, and the user cannot do the thing — "I can not find a way to do that" | grep for a call site, not for the function: `patchClass` existed in `teacherApi.ts` with a green test and ZERO callers in any UI | **manual** (1.1.112 2026-09-14; **second instance 1.1.123 2026-09-21** — ALS-SHARE M3b's researcher edit/unpublish/delete on any activity shipped as a *backend* milestone on 2026-06-26 and had no call site for three months, while three pages and the researcher guide said *read-only*; JB asked whether it was possible) — `PATCH /api/classes/{id}` has accepted `{name, description, cohort}` since the permission model landed, so a teacher who mistyped a class name had no way to correct it for months. A test on the client function proves the client function. ⚠️ The tempting workaround is worse than the fix: duplicate-and-delete re-mints group codes (killing every join link already handed out) and splits one class's logs and rubric runs across two `class_id`s — append-only is right for *claims*, not for a label on a container |
| **A bare scalar next to `UploadFile` is a QUERY parameter** | the client sends `skill_id` as a multipart form field, FastAPI reads `skill_id: str = ""` from the query string, the field is silently `""`, the upload returns **200**, and the file never appears — because the list filters on the field that was dropped. Each half has green tests; only the join fails | declare form fields `Annotated[str, Form()]`; and for any upload→list pair, one test that drives BOTH routes against ONE store with the frontend's exact field name (`test_upload_reaches_the_workbench_list.py`) | **enforced for documents** (1.1.121, 2026-09-16) — every workbench upload since the fork's initial commit was stored with `skillId=""`; 14 orphan rows in prod across three student groups. ⚠️ **The triage was wrong twice because of a second footgun:** the "zero requests reached the route" finding was a log query filtered on the *group*, a field the access-log line does not carry — the same "a checker answers when it could not read its subject" shape as `deploy-status`. When a server-side log query returns nothing, re-run it **unfiltered** before concluding the request never arrived |
| **Blocking I/O inside an `async def` route** | a researcher opens a dashboard and a student's tutor reply stalls for a minute with no error — every stream on the instance freezes, because uvicorn runs `--workers 1` and the loop is blocked. The stalled route itself looks merely slow | synchronous BigQuery / heavy Firestore work in an `async def` goes through `await asyncio.to_thread(...)` (`CACHE.aget_or_compute` for insights); `db.bigquery.run_query` logs `run_query on the event loop` with the caller's stack if you forget; `test_blocking_queries_off_the_loop.py` drives the real route beside a `/ping` | **partly enforced** (1.1.131, 2026-09-22) — `insights/compare?scope=all` held the loop 59 s during a 1st-year class; the student re-sent. The runtime log catches BigQuery only; a long synchronous Firestore scan in an `async def` is the same bug and nothing flags it |
