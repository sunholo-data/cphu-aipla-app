...
async def test_save_then_load_roundtrips_bytes_and_mime():
    ...
    part = await load_activity_image(**_KW, mime_type="image/png")
...
async def test_load_missing_returns_none():
    part = await load_activity_image(teacher_uid="t", activity_id="a", material_id="nope", mime_type="image/png")
    assert part is None
...
async def test_delete_removes_slot():
    ...
    await delete_activity_image(**_KW, mime_type="image/jpeg")
    assert await load_activity_image(**_KW, mime_type="image/jpeg") is None
...
async def test_distinct_activities_do_not_collide():
    ...
    p1 = await load_activity_image(teacher_uid="t", activity_id="a1", material_id="m", mime_type="image/png")
    p2 = await load_activity_image(teacher_uid="t", activity_id="a2", material_id="m", mime_type="image/png")
