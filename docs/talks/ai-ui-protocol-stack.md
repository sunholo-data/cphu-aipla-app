...
| **A2A** | `/.well-known/agent.json` agent card; agent-to-agent JSON-RPC | `backend/protocols/a2a.py` ported from v5 Sunholo | Probably yes — v5's A2A predates A2A 1.0 | None — `/.well-known/agent.json` endpoint is design-frozen, TODO at `backend/fast_api_app.py:86`. The card we'd publish has NEVER been validated against the canonical A2A schema. | ❌ unverified — file an issue, write a card-shape contract test before workshop |
