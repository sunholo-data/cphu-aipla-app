...
| 4 | Strand C scoping note delivered | ✅ **DRAFTED 2026-09-02**, seven days early | [strand-c-scoping-note.qmd](strand-c-scoping-note.qmd). Written as findings rather than proposals — all three directions were partly built, so it reports results. Carries one real negative result (C2 transcription) and one unplanned finding (C3's granularity is capped by the anonymity decision). **Wants a read from JB/AR before it counts as delivered** |
