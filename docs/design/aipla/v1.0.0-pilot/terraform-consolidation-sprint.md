...
- **RESOLVED (M, 2026-07-27): test seeds ALL dev content — no copyright gate on test.** M's call: test is environment/infra hardening, not a student-facing shared corpus, so the copyright-clearance gate (A-level cleared; B/C + 2010 exam archive NOT) applies at **prod**, not test. Seed test's corpus from dev's full content. **Prod seeding stays cleared-only — enforce there, and do NOT promote test's corpus to prod.**
- **2026-07-27 — M3a DONE.** All pre-deploy secrets populated on test: `AGENT_ENGINE_ID`=6758733160311160832 (europe-west1), `CURRICULUM_RAG_CORPUS_NAME`=…/ragCorpora/4611686018427387904 (empty corpus), `DOCPARSE_API_KEY`=dev's value. **M4 firing:** tag `v0.1.0` (first official release; dev was all pre-release iteration — M) → `aipla-test-release` build+deploy.
- **FOLLOW-UP (M, 2026-07-27): versioning cleanup.** `docs/design/v6.x.x/` are inherited-template relics — relocate/archive + fix cross-links (incl. 1.3b/3.2's ref to `v6.0.0/implemented/infra-terraform-lessons.md`). Keep two axes distinct: design-doc milestone folders (`v1.0.0-pilot`…) vs git release tags (`v0.1.0`=first release). Separate deliberate task, NOT mid-deploy.
- **2026-07-27 — M4 + M5 DONE. Test env LIVE and verified.** `v0.1.0` deployed at https://aipla-v01-frontend-y2bmxayxca-lz.a.run.app; **auth-gap verdict GREEN** (auto-seed job ran clean, no 403). Smoke core-green (sandbox is the one known gap — deferred). E2e round-trip PASS: join `aipla-demo-1` → tutor turn → **BigQuery** (messages=1). Two more placeholder-URL bugs fixed en route (both hardcoded `*-placeholder.a.run.app` from before test existed): `seed-demo-codes.sh` → derive URL dynamically; `cli/aiplatform/http.py` test URL → real. Operator impersonation (`m@sunholo.com` → `tokenCreator` on runtime SA) folded into Terraform (`admin_operator_members` var) instead of a manual gcloud grant. `identity_platform_config` perma-diff closed (declared the disabled email/phone/multi_tenant sub-blocks). **Remaining (optional for "infra hardening"):** deploy test sandbox (M1-remainder) + teacher-SSO round-trip + ≥24h soak.
- **2026-07-27 — M4 attempt 1 FAILED (frontend), fixed.** CI-gate PASSED + both images started (so lint/tests/`_IMAGE_TAG` all good); build died at frontend prerender: `/dev/mcp-apps/active` did `new URL("")` because `MCPAppToolCallRouter` used `?? "localhost"` (doesn't catch empty) and test passes `_MCP_SANDBOX_URL=""`. Fix: `??`→`||` (empty falls back too) — a latent bug that would bite any sandbox-less env incl. prod. Verified locally (`NEXT_PUBLIC_MCP_SANDBOX_URL="" npm run build` → 26/26 pages prerender). Re-cutting `v0.1.0` on the fix (attempt 1 deployed nothing — died before the deploy step).
