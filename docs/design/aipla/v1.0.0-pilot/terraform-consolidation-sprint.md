...
- **NEXT: G1** — human console step (below), then M2 (import + apply).
...
### G1 — Human gate: install the GitHub connection + confirm Firebase `(human, ~0.5d wall)`
- Install/authorize the `sunholo-github` 2nd-gen Cloud Build connection in `aipla-test-2026`/`europe-north1` (GitHub OAuth — **console/manual**; TF can create the connection resource but the GitHub App authorization is completed by a human). Link the `cphu-aipla-app` repo.
- Confirm Firebase can be added to the project (google-beta `google_firebase_project`, or `firebase projects:addfirebase` if TF ordering fights the enablement).
