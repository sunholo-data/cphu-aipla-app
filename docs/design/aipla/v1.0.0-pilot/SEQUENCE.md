...
| 1.H | [audio-capture-and-tts.md](audio-capture-and-tts.md) — *brief:* [`audio-capture.md`](../_scoping-snapshot/prototypes/audio-capture.md) | P2 | 0.5d TTS + 2d audio | TTS none; audio capture gated on **JB sign-off (5 consent / privacy questions)** + 1.A for per-class opt-in | **Split implementation.** TTS (browser-native `speechSynthesis`) ships independently — zero privacy gate, ~0.5d. Audio capture (opt-in group recording for research) blocked on JB consent / institutional approval sign-off — five questions in the brief must be answered before any code merges. Audio embodies SECURE-BY-CONSTRUCTION by structurally refusing to ship without consent decisions |
...
| 1.3b | [terraform-consolidation.md](terraform-consolidation.md) | **P1-infra — this month's focus (2026-07-27)** | ~1wk (inc-2 + dev import to zero-diff + config extraction) + the 1.3 test/prod cut | completes 1.1 (increment 2 + dev import); **supersedes 1.3's "dev stays imperative" clause**; feeds 3.2 Layer 1 | **One IaC source of truth.** Finishes increment 2 (Firebase + Cloud Build triggers + the `serviceAccountTokenCreator`-on-self binding that closes the 1.1 SA auth gap), folds **dev** into Terraform via `terraform import` to a zero-diff `plan` (retiring `bootstrap-aipla-dev.sh` as authority), extracts runtime config out of `cloudbuild.yaml`, and shapes the module boundary by capability so the on-prem Layer-2 ([3.2](../v2.0.0-handover/self-hosting-and-terraform-handover.md)) mirrors it 1:1. Unblocks the test/prod cut (1.3). Anti-goal: no premature multi-cloud HCL |
