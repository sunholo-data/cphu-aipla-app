...
| 1.3 | [env-promotion-dev-test-prod.md](env-promotion-dev-test-prod.md) | P1-infra | 0.5d doc + per-env provision | 1.1 (Terraform module makes a new env "a tfvars file"); deploy CI-gate (landed 2026-06-17) | **Cut test/prod from dev.** Promotion flow (ff-only dev→test→prod, every env deploy gated by `ci-gate-*`) + per-env readiness checklist + cut order. test/prod projects exist but are **bare** (Secret Manager not even enabled). Trigger: the 2026-06-17 dev-only demo outage — a `test` tier must exist before the 2026-08-14 pilot. Doc landed 2026-06-17; not built |
