...
- The scoping site ([architecture.qmd ADR-001](../../_scoping-snapshot/architecture.qmd#adr-001-student-identity-no-auth-anonymous-group-ids) and [strands.qmd](https://aipla.ku.dk/project/workstreams) `manage-class` row) decided teachers get UCPH SSO *or* Firebase federated; the v1 scope commits to `manage-class` as a teacher skill. The auth-mechanism choice is the "open question for JB" still flagged in the ADR.
...
- [ADR-001](../../_scoping-snapshot/architecture.qmd#adr-001-student-identity-no-auth-anonymous-group-ids) — anonymous group IDs (this doc's parent decision)
- [strands.qmd](https://aipla.ku.dk/project/workstreams) — `manage-class` skill row in the v1 catalogue
