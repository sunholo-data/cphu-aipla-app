...
| 7 | 100,000 DKK on cloud costs | **Planning input** | ⚠️ Whether this is a budget, a cap, a grant or a forecast is **not stated in the notes** and changes what to do with it. Ask before planning against it |
| 8 | Costs for a Claude-Code-lite per student vs teachers | **Analysis — not a build** | Everything needed to compute it ships: per-turn token+cost telemetry in BigQuery, `/teacher/insights/cost` ([1.1.9](cost-dashboard.md)), and the per-payer cap + programme-wide daily budget ([1.1.76](delegated-programme-administration.md) M3, shipped 09-03). **What is missing is the arithmetic, not the data** — a per-seat-per-term forecast from the pilot's actual distribution. Sits beside [1.1.94](session-resource-transparency.md), which reports the same numbers in J/W/kWh to a different audience. ~0.5d of analysis, no code |
...
5. **What the 100,000 DKK figure is** (item 7) — budget, cap, grant or forecast.
