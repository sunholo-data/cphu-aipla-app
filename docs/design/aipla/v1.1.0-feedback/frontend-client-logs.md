...
|---|---|---|---|
| `app_error` / `app_global_error` | ERROR | **new** `app/error.tsx` / `app/global-error.tsx` — neither exists today (verified) | A render throw anywhere in the tree currently shows Next's default screen and logs nothing. `digest` joins to the server log for a Server Component error. The boundary does **not** print `error.message` on screen (it may quote a student's document or message); the log has it |
...
| Next.js `app/error.tsx` / `global-error.tsx` | **absent** — Next's default error screen | **add** both; `global-error.tsx` must render its own `<html>`/`<body>` (Next requirement) and has no access to the app's providers, so it uses the logger's *unauthenticated hold* path and flushes on the next navigation |
