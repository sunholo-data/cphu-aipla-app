...
**Status:** ⚠️ **PARTLY SHIPPED despite the deferral** — a first slice landed (`feat(sim-bridge): deep-link CTA — 'Open in AIPLA', external-host only (1.1.55 first slice)`; `reportSize + initialState`). The *capture* half remains deferred. Originally: **DEFERRED / backlog** — decision 2026-07-08 (M): *external-host capture is **not currently scoped**; the `callTool` round-trip is a new channel we may want to use, so this is **captured for a future discussion**, not scheduled.* Confirms this doc's own §11 open question ("AIPLA app for research, external hosts for dissemination" is the line — for now). No code to be written until that discussion reopens it. **Likely direction when it does — §0:** external hosts as an *advertising/discovery* surface with a deep-link CTA into the deployed app for full teaching+support (which sidesteps the identity/consent problem). (Prior status: PROPOSED design exploration.)
