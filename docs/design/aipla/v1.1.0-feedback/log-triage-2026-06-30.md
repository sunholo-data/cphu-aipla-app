...
**Goal:** firm up *existing* features (per the UX-coherence gate (`project_ux_coherence_gate.md` — agent-memory note, on M's machine)) — no new features.
...
1. **Bug #1 (P1) first** — it's the platform-robustness keystone and scales badly into the pilot (every lapsed student token = a silently-dead session + backend spam). Fix (a)+(b) is a focused frontend change (`apiClient.ts` + `CallTeacherButton.tsx`), testable with a forced-expired-token vitest. Pairs with the anonymous-users corner-case (`feedback_anonymous_users_are_corner_case.md` — agent-memory note, on M's machine) history.
