...
**Status**: **PARTLY SHIPPED — 1.1.91.** M0, M1 (store + researcher tier), M5 (**7 of 7 frameworks**, all drafted from source 2026-09-10) and M7 are **shipped**: M0 and M1a reached production in `v0.1.40`; M5 and M7 in `v0.1.41`. **M2 (the co-pilot) shipped to dev 2026-09-11**, as did **M1's teacher tier** (as [1.1.110](custom-approaches-and-literature-corpus.md) custom approaches) and **structural construct editing**. **M6 is closed by [1.1.111](register-belongs-to-the-approach.md)**, which removed the clash rather than policing it. **M3 (preview/compare) and M4 (researcher cross-view) both shipped to dev 2026-09-11.** **Every milestone in this design is now shipped.** Per-milestone state in the table below — read it before planning anything here.
...
| M2 | **Tutor co-pilot** on the shipped shell | ~2d | **✅ SHIPPED (dev) 2026-09-11** — five propose-tools (`propose_approach`, `propose_behaviours`, `draft_approach_prompt`, `critique_approach`, `find_source_passages`) on the shared shell, mounted on the Approaches surface. It did land on the structural editor rather than a text box, as this row predicted. **The never-invent-a-citation requirement is enforced in three layers, none of them a prompt instruction**: the type system (`vouched_by` has no default), the tool boundary (`_strip_provenance` removes every citation field a model attaches, in any shape), and one grounded path in (`find_source_passages` retrieves passages and pairs them with the citation already vouched in that approach's YAML — the model picks the passage, never the source). The worst a confabulating model can do is propose a shape with an empty source list. Researcher gate resolved server-side from the Firebase claim, **three-valued** so an unreadable claim refuses differently from a real refusal. Apply does NOT write — it opens the editor, keeping the researcher-gated `PUT .../structure` the only path to a stored change |
| M3 | Preview + side-by-side comparison | ~1d | **✅ SHIPPED (dev) 2026-09-11** — `POST /api/research/tutors/preview` runs one scratch turn against up to TWO tutors on the caller's own turn, and `TutorPreviewPanel` renders the answers side by side. **Teacher-accessible, not researcher-only**, because the 09-09 second audience is exactly teachers. Turns are logged under `preview:{uid}` **content-free**, so spend stays visible while the researcher lens excludes them by construction — a preview is a real tutor turn carrying a real `framework_id`, and unmarked it would read as classroom evidence with nobody taught. The prompt is COMPOSED from the same pieces a lesson uses (skill instructions + `resolve_framework_instruction`), never paraphrased, and the response states what a preview does NOT carry (activity materials, teacher ILOs, group history) rather than implying parity |
| M4 | Researcher cross-view over teacher-authored tutors | ~1d | **✅ SHIPPED (dev) 2026-09-11.** **Gate met** — M confirmed teachers know researchers view this, and the trust-card half is now IN THE PRODUCT: the custom-approach panel tells a teacher that researchers can see their approaches and their name against them, and that they cannot change them. `GET /api/research/frameworks/crossview`, researcher-only, tags the span `auth.researcher_bypass`. ⚠️ **Intent and use are separate columns** — tutors that NAME an approach vs turns actually taught — and a blank turn count means the chat log was unreadable, never zero |
