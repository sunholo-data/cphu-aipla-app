...
**Status:** **PRE-PILOT COMPLETE — M0–M3 SHIPPED 2026-06-22.** A teacher attaches a vetted sim to an activity, a student sees it render, and the tutor gets the sim's mechanics (`tutorBlock`) composed with the activity goal — **end-to-end**. The 3 `tutorBlock`s are placeholders pending **AR** (content only — the mechanism is live; refining them is a YAML edit, no code). **M4 legacy migration is post-pilot.** (**P1** — additive, the 3 legacy sims coexist.)
...
**The "marketplace."** For v1.1 this is a **curated library** — the 3 live artefacts + the [jitt-dk 23](../v1.0.0-pilot/jitt-dk-artefacts.md) as they onboard, each ADR-013-gated. The **middle layer** — a runtime CMS where AR/researcher edits `tutorBlock`s + onboards deployed sims without a deploy — is [sim-catalogue-admin.md](sim-catalogue-admin.md) (1.1.42, post-pilot). An *open* marketplace (third-party contributions + a review queue) is the Year-2 top layer that connects to [post-pilot teacher-artefact-authoring (2.4)](../post-pilot/teacher-artefact-authoring.md); the manifest + the ADR-013 gate are its backbone.
...
| **M2** ✅ | **Prompt composition — SHIPPED 2026-06-22.** `compose_teacher_focus()` prefixes the activity's `teaching_goal` with the hosted artefact's `tutor_block` (resolved by `artefact_id`) inside the shipped `{teacher_focus}` injection — so the **same** sim tutors differently per activity purely because the goal differs (regression-tested: two activities, one artefact, different goals → different composed prompts; both carry the shared block). Graceful fallback for a de-catalogued/block-less artefact. **Q1 resolved: reused `concept-dialogue` + `{teacher_focus}`** (no new skill). `tutorBlock` *content* still AR-placeholder; mechanism live. | ~1.5d | AR (content only) | **shipped** |
