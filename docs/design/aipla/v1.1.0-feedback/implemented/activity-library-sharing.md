...
**Status:** Implemented (M0–M3, 2026-06-26) — **PRE-PILOT (pulled forward 2026-06-24, M)**. The pilot runs in **research mode with a trusted teacher set**, where cross-teacher see/share/adopt-copy *is* the pilot behaviour, not a later marketplace. The M3 sharing-scope gate was **resolved** (see ↓ "Sharing decision"). M0–M1 shipped as the ALS-1 sprint; M2 (duplicate), M3 (publish + shared catalogue + adopt), researcher CRUD-over-all, and the light History provenance panel shipped as the **ALS-SHARE** sprint (see `implemented/activity-library-sharing-share-sprint.md`). **M4** (adoption attribution / "most-adopted") stays optional/deferred.
**Last Updated:** 2026-06-26 (ALS-SHARE sprint complete — M2 duplicate, M3 publish/catalogue/adopt, M3b researcher CRUD, M-HIST provenance panel)
