...
**D1 — Is the Google data agreement in place? UNKNOWN, and finding out is the
action.** The notes read both ways within two minutes. Consequence: everything
student-facing sequences behind an unresolved gate, and *"establish the status"*
is the highest-lead-time action on the list. **Do not plan November against a
date whose precondition nobody has confirmed.**
...
| 4 | Researcher sees what students did, like a teacher can | **MOSTLY SHIPS** | Verified in code: `/teacher/classes` has a researcher `scope=all` toggle; **every class GET uses `_load_readable` (owner *or* researcher)** while writes stay `_load_owned`; cards link through to class detail and group reports. **First check whether the `role:researcher` claim is granted on the environment in use** — claims are per Firebase project and need a token refresh. Not a build |
...
| 11 | Tutors from frameworks (ESRU, SDT, Dysthe, authentic questions, student discipline); configurable for researchers; persona workflow with previews | **New build** | → **[1.1.91](researcher-configurable-tutors.md)**. `adk/interaction_style.py` (1.1.20) ships as the primitive; the **persona bundle does not exist in code** (one comment references it). `adk/authoring_framework.py` M0 ships but its prompt is **explicitly a placeholder behind a human gate** — that gate is this item |
...
| 14 | Etkina scientific abilities / rubrics; multiple representations | **Content decision for AR** | Feeds 1.1.92 as *a* rubric, not as its precondition — that is the whole point of D3. ⚠️ Name unverified, see [Terms](../../../notes-2026-09-01.md#terms-i-inferred) |
...
| **1.1.91** | [researcher-configurable-tutors.md](researcher-configurable-tutors.md) | **P1** | ~3–4d | Framework *content* from JB/AR; the mechanism is un-gated |
