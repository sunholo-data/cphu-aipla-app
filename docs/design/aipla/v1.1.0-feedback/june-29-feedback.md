...
- **A large Strand C cluster** — theoretical framework (embodied cognition → resources framework), social-network / baseline-longitudinal study design, causal-inference methodology (CCM, side-effects, observer effect), and the video-coding instrumentation that continues the 23-June thread. Belongs in the **scoping site's Strand C**; **flagged here, not yet folded in** — the largest to-process chunk, and the natural agenda for the post-freeze Strand C scoping kickoff (Phase 2.2). **Update (2026-06-30):** the *knowledge-graph + mastery-monitoring + cross-group-matching* slice (the part M was most excited about) now has an execution design sketch → [../post-pilot/knowledge-graph-and-student-matching.md](../post-pilot/knowledge-graph-and-student-matching.md) ([SEQUENCE 2.9](SEQUENCE.md)); the research framing stays in Strand C.
