...
**Status:** DONE (2026-07-10) — code + tests + build green **and browser-verified live on deployed dev** (revision 00545; the co-pilot is un-dark-flagged on dev via `_AUTHORING_COPILOT=1`): clean cards (no empty tool-call bubbles), lesson/element proposals, and `attach_material` with curriculum summaries all Apply-round-trip. All acceptance items met. The fast-follow from [teacher-coworking-copilot.md](teacher-coworking-copilot.md) Part 1. The shared shell shipped and is reused by the class + analytics co-pilots; the **activity-authoring** co-pilot has now been folded back onto it (the duplicate chrome is deleted). One gap surfaced during the code read that the recon below missed: the card's inline-Edit textarea hardcoded `aria-label="Edit proposal"` (English) while the authoring suite asserts the Danish `Rediger forslag` — fixed by adding `editAriaLabel` to `CopilotLabels` (defaults to "Edit proposal").
...
- ✅ Browser-verified on **deployed dev** (2026-07-10, revision 00545): proposals render with clean cards (the empty-content tool-call bubbles are gone), and Apply round-trips — including `attach_material` proposing curriculum docs (with backfilled summaries) into Materials. Verified live by M on the deployed dev URL, not just against mocked tool results.
