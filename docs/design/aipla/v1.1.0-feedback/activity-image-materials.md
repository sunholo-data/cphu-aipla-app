...
**Status:** **DESIGN.** Net-new. Lets a teacher attach an **image** (a diagram, a graph, a photographed worksheet) to an activity so the **tutor sees the pixels** during the student conversation — not OCR'd text, the actual image as multimodal context. (**P1** — additive; deepens the [1.1.25 Materials picker](curriculum-library.md) + the [1.1.41 resource](teacher-sim-resources.md) axis.)
