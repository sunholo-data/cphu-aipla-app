...
**Status:** **COMMITTED / IN BUILD** — M opened the UX-coherence gate on 2026-07-10 ("we want a demo up so we can start iteration"). Sprint **CONCEPT-1** ([concept-map-sprint.md](concept-map-sprint.md)) shipped the demo slice to dev on 2026-07-11: M0 element (both stores) + M1 list-editor/graph/student view (`NEXT_PUBLIC_CONCEPT_MAP`) + M2 co-pilot `propose_concept_map` diffs + the **chat-native checkpoint slice pulled forward from M2 here** (`run_checkpoint`/`record_checkpoint`, per-group `concept_progress`, CheckpointCard, map light-up). **M2's passive `mark_concept` check-off shipped 2026-09-25** in [CONCEPT-2](concept-map-steering-sprint.md) — on an append-only evidence record, with precedence `teacher > checkpoint > observed`, and with the teacher override this doc's own prompt text has promised since CONCEPT-1. Still open from this doc: the **post-turn reconciling judge pass** (deliberately deferred — this doc's own warning against shipping that depth as a casual probe stands until there is calibration evidence) and the M3 eval/calibration gate. The teacher coverage surface is now the class-level graph (1.1.121 / CONCEPT-2 M5).
