...
- **Q1 — tabs vs activity-driven primary surface.** The mockups show the workbench right-pane as *one* surface per activity (viewer OR editor). Do we (a) always tab (Arbejde/Dokumenter/…) or (b) let the activity pick a primary surface with tabs only when >1 exists? Recommendation: **(b)** — primary surface by activity, tabs when multiple; least clutter, matches the mockups.
