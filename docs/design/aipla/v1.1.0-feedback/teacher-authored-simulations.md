...
**Priority:** **P2 for the pilot; P1 for handover.** No teacher path can be built and hardened before 2026-08-14, and the pilot works without one (three curated sims + the element palette). It matters at **handover (2026-09-15)**: a platform where only M can add a simulation has a bus factor of one. *(2026-08-27: the engagement extends to at least April 2027, so the 2026-09-15 cliff is softer — but the bus-factor argument is about the platform, not the calendar, and stands unchanged. If anything the extension makes this MORE worth building, since there is now time to build it properly rather than to document the gap.)*
