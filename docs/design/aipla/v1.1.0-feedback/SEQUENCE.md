...
| 1.1.27 | [lesson-author-surface.md](lesson-author-surface.md) | **P1 — NEEDS RE-SCOPE** | ~4d as-written (overlap likely reduces it) | [1.G teacher-ui](../v1.0.0-pilot/implemented/teacher-ui.md) shipped; ADR-004 AILANG Parse; **JB sign-off** on bounded-knob set | **Drafted 2026-06-05; renumbered from 1.1.17→1.1.27 on 2026-06-12** (the original 1.1.17 was reassigned to `student-engagement-signals` upstream on 2026-06-06). ⚠ **Overlap review needed (M):** drafted *before* the 9-June batch, this doc's four capabilities now substantially intersect shipped/in-flight rows — (2) bounded knobs ↔ **1.1.20** `interaction_style` presets (SHIPPED); (3) teacher curriculum upload ↔ **1.1.25** curriculum-library (PARTIAL) + **1.1.19 M6** materials picker; the screen itself ↔ **1.1.26** teacher-ui-consolidation (SHIPPED). **Net-new and not yet covered:** (1) **resolved-prompt preview** (read-only assembled system prompt with source-attribution badges) and (4) **trial session** ("Try this lesson", `is_trial:true`, excluded from analytics/research). Recommend re-scoping this row down to just (1)+(4) and folding the rest into their now-canonical rows, or marking SUPERSEDED. **Absorbs parent SEQUENCE row 1.9** `concept-dialogue-config`. Net axiom score (as originally drafted) **+7** |
...
**Tally (incl. 9 June batch + 2026-06-11/12 sessions): 16 SHIPPED · 11 OPEN.** (1.1.26+1.1.20 2026-06-09/10; 1.1.12 2026-06-10/11; 1.1.7+1.1.21+1.1.23 shipped 2026-06-11; 1.1.25 M1 2026-06-11, M2–M5 + full sprint complete 2026-06-12.)
