...
| 1.1.16 | [security-monitoring-pipeline.md](security-monitoring-pipeline.md) | P1 | ~1d (0.5d CI gate + cron, 0.5d skill) | None — additive on existing `ci.yml` | **New 2026-06-05** — Prompted by the 20-alert dependabot pile that surfaced on 2026-06-05 (commits `084920b` + `1257c08` patched all 20). Three-layer pipeline: (1) CI gate fails PRs on new HIGH/CRITICAL in production deps across frontend/sandbox/backend (npm audit + pip-audit + OSV); (2) Weekly cron updates a rolling tracking issue with the full audit + dependabot alert summary; (3) `aipla-security-checkup` skill encodes the human triage rubric. Designed to be the forcing function the prior posture lacked — gates merges, surfaces weekly cadence, codifies the reachability classification used in the 2026-06-05 triage. Net axiom score +4 (SECURE BY CONSTRUCTION +1, EARNED TRUST +1, OBSERVABLE BY DEFAULT +1, PROTOCOL OVER CUSTOM +1; no conflicts) |
