...
- [x] P1.3 — **Shipped 2026-07-23 (pending first-deploy verify).** Added `admin.platform_seed.main()` (a `python -m admin.platform_seed` entrypoint that runs `seed()` in-process via ADC — no HTTP, no token mint — exits 1 on any failed template, 2 if misrun under LOCAL_MODE; 3 unit tests). `scripts/deploy-seed-job.sh` create/updates + runs the `aipla-seed-skills` Cloud Run job as `aipla-v6@`; `cloudbuild.yaml` calls it as the last deploy step so a bad seed reds the build. Key finding: **no new IAM** — the build already deploys the service as `aipla-v6@` (needs `run.admin` + `actAs`), which suffices to create+run the job as the same SA. `make seed-job` for the laptop path. The old HTTP `make seed` stays for no-deploy seeds. Verify on the next `dev` build (job gets created on first run; if the seed step fails, `make seed-job ENV=dev` diagnoses).
