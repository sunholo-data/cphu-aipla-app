...
|---|---|---|---|
| 1 | Tutor ignores uploaded assignment files; cites a different Question 5 | **Defect (design-level)** | **M1+M2 SHIPPED 2026-08-31** → [1.1.87 activity-task-materials-in-context.md](activity-task-materials-in-context.md). A material is now attached as **Reference** (RAG) or **In context** (the tutor is handed the full text every turn); the eval + the teacher rebuilding their exam activity are the remaining gates. Original triage: **D1 ANSWERED 2026-08-27.** `MaterialRef.kind` decides: `image` gets the ADK artifact path (the tutor HAS it), `curriculum` gets a retrieval tool it may choose to call. A PDF is `curriculum`. The fix is the missing third loader/injector twin. |
...
| 13 | Several instances of one element (two tables) | **SHIPPED 2026-08-31** — [1.1.71](multi-table-activities.md) M1–M3. Up to 5 tables, charts pick which one they plot | [multi-table-activities.md](multi-table-activities.md) (1.1.71), designed 2026-08-10, deferred during pilot week for id-migration risk. **Third independent request** (Aswin 10 Aug · 25 Aug notes · this teacher). Shares a root cause with item 26. |
| 14 | Drag table columns to reorder variables | **New build** | **BUILDER CLUSTER.** |
| 15 | Upload fails: picker hides screenshots, then errors | **HALF FIXED** | The error is defect A (1.1.79, promoted). The **picker half is open**: `StudentDocumentWorkbench`'s accept list has no image types, so "Custom Files" genuinely hides screenshots. ~~One-line fix~~ — **NOT a one-line fix; corrected 2026-08-27 in [1.1.85](student-upload-consolidation.md) M1 and re-confirmed against the code 2026-09-01.** The frontend accept list is an *honest mirror* of the backend's `_ALLOWED_EXTENSIONS`, which carries no image types and 400s anything else, and the refusal is deliberate (`upload.py`: *"Images aren't accepted at upload — they go through the solution element as pixels, not the parse route"*; AILANG Parse returns `None` for images by design). **Widening `ACCEPT` alone would turn today's silent OS-picker filter into a server rejection — strictly worse, and precisely the "then errors" half of what this teacher reported.** M1 is a routing change on both sides, ~0.5d. Do not pick this up as filler. |
...
| 17 | Asterisk as a multiplication sign | **SHIPPED 2026-08-27** | `skills/preambles/math_notation.md` + `adk/math_notation.py`, injected **unconditionally** into every skill's prompt — a house style with no flag to forget. Takes effect on **DEPLOY, not seed**: `platform_seed` globs only `skills/templates/*/SKILL.md`, and the preamble is read from disk at agent-build time. |
...
| 26 | Groupmates overwrite each other's table data; AI sees only the latest | **M1+M2 SHIPPED 2026-08-31** | → **[1.1.88 group-shared-table.md](group-shared-table.md).** Sharper than "revisit July": `WorkbenchTable` is the **last fillable element still on `sessionStorage`** — checklist, concept and writing all moved to per-group stores, and `writing_progress.py` names the table as the one left. Shares the `table.state` single slot with item 13. |
