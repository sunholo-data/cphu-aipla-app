...
| **1.C** | [aipla/v1.0.0-pilot/implemented/led-planck-skill.md](v1.0.0-pilot/implemented/led-planck-skill.md) (technical execution); [`led-planck-skill-brief.md`](_scoping-snapshot/prototypes/led-planck-skill-brief.md) in scoping site (pedagogical source-of-truth) | **Second physics skill.** Self-contained Danish stx artefact (~1855 LOC, zero external fetches). Procedural-virtual-lab artefact class — different form factor from Boldkast's phenomenon-sim. In-repo design doc covers file paths, axioms, ADR-013 scan, test plan; brief covers tutor prompt + lesson pedagogy | 013 | 1.5-2d |
| **1.D** | [aipla/v1.0.0-pilot/implemented/kinebot-migration.md](v1.0.0-pilot/implemented/kinebot-migration.md) (technical execution + runbook commitment); [`kinebot-migration-brief.md`](_scoping-snapshot/prototypes/kinebot-migration-brief.md) in scoping site (audit + pedagogy) | **Third physics skill — canonical AIPLA migration runbook.** External AI artefact (1707 LOC) built by DK, has direct Anthropic API calls in browser that AIPLA must strip. NCERT/CBSE Class 11 kinematics, English. Beta cohort: ~100s of DK's Indian students. **The in-repo design doc commits to landing the migration as a permanent runbook section in `.claude/skills/mcp-app-artefact/` for future external-artefact onboarding** + adds `aiplatform artefact audit` CLI command for the dogfooding loop | 013, 014 | 2-3d |
| **1.E** | [aipla/v1.0.0-pilot/implemented/workbench-state-debounce.md](v1.0.0-pilot/implemented/workbench-state-debounce.md) (execution); [`workbench-state-debounce.md`](_scoping-snapshot/prototypes/workbench-state-debounce.md) (brief) | **Quick fix from 2026-05-25 meeting** — slider drags spam the chat with cards. Move debounce-at-host (500ms shipped in MCPAPP-SPEC) to debounce-at-artefact (800ms) + coalesce-by-field-at-host (300ms). Wire shape becomes minimal-delta (`{changed, value, unit}`) not full snapshot. Centralises in `StaticArtefactFrame` so future artefacts inherit | — | 0.5d |
| **1.F** | [aipla/v1.0.0-pilot/implemented/session-persistence.md](v1.0.0-pilot/implemented/session-persistence.md) (execution); [`session-persistence.md`](_scoping-snapshot/prototypes/session-persistence.md) (brief) | **Same group code resumes the same session for 30 days** — cross-device coherence, chat history + workbench state restored on rejoin. Adds `aipla:restore` artefact contract required by all current + future artefacts. Adds `aiplatform sessions resume / reset` CLI for ops parity | 001, 005 | 1.5-2d |
| **1.G** | [aipla/v1.0.0-pilot/implemented/teacher-ui.md](v1.0.0-pilot/implemented/teacher-ui.md) (execution, **phased**); [`teacher-ui-brief.md`](_scoping-snapshot/prototypes/teacher-ui-brief.md) (brief) | **P0 — demo target Wed 3 June.** **Compressed via 3-phase split (2026-05-25 evening decision):** Phase 1 = static mockup ASAP (~0.5-1d, no backend), Phase 2 = wire to real backend with LOCAL_MODE teacher stub (~2-2.5d, Wed 3 June demo state), Phase 3 = Firebase + 1.A swap + stretch (~2-2.5d, post-demo). **Unblocks 1.A from being on the critical path** — 1.A now runs in parallel with Phase 2. See teacher-ui.md "Phased delivery" section for per-phase acceptance gates | 001, 005, 014, 015 | 5-6d (split) |
| **1.H** | [aipla/v1.0.0-pilot/audio-capture-and-tts.md](v1.0.0-pilot/audio-capture-and-tts.md) (execution); [`audio-capture.md`](_scoping-snapshot/prototypes/audio-capture.md) (brief) | **Split implementation.** TTS (tutor responses read aloud via `window.speechSynthesis`) ships independently — zero privacy gate, ~0.5d. Audio capture (opt-in group recording for research) blocked on **JB sign-off** on 5 consent / privacy / retention questions. Per-class teacher opt-in toggle in 1.G | 001, 005 | 0.5d + 2d |
...
| ~~**1.9**~~ | ~~`concept-dialogue-config-skill.md`~~ | **Superseded by [v1.1.0-feedback/teacher-activity-authoring.md](v1.1.0-feedback/teacher-activity-authoring.md)** (row 1.1.19) + [tutor-personas.md](v1.1.0-feedback/tutor-personas.md) (1.1.20, socratic `interaction_style`, SHIPPED). The chat-only Socratic tutor ships as 1.1.19's `none`/chat activity type configured with the socratic style — the original 1.9 "A2UI config form" framing. *Visibility into* the resolved prompt + a trial session is the separate [lesson-author-surface.md](v1.1.0-feedback/lesson-author-surface.md) (row 1.1.27, re-scoped 2026-06-12). The original ~1.5d estimate folds into 1.1.19. | — | — |
...
[`june-03-feedback-sprint-brief.md`](_scoping-snapshot/prototypes/june-03-feedback-sprint-brief.md)
...
[`june-09-feedback-sprint-brief.md`](_scoping-snapshot/prototypes/june-09-feedback-sprint-brief.md))
...
> ([`teacher-analytics-framework.md`](_scoping-snapshot/prototypes/teacher-analytics-framework.md)
...
| 2.5 | [aipla/post-pilot/session-analytics-rubric.md](post-pilot/session-analytics-rubric.md) | **Committed v1 — promoted 2026-05-28** (teacher monitoring + analysis raised above original scope; must be live *for* the pilot, not built on its aftermath). Gated on 1.2 + the JB/AR framework sign-off. | The analysis layer over the stored logs — turns "34 messages, 8 sim runs" into engagement + concept signal. **Framework direction settled 2026-06-03 in the scoping site's [`teacher-analytics-framework.md`](_scoping-snapshot/prototypes/teacher-analytics-framework.md) brief**: DRA-led (the (b) **CPS + DRA** stack from the earlier framing) — Linder/Bruun/Pohl/Priemer 2024 representational-competence framework, consuming the [1.K](v1.0.0-pilot/dra-activity-framework.md) DRA maps as machine-readable input. The scoping-site brief targets **v1.2 analytics chat**, not v1.0 pilot — which softens the original "must be live *for* the pilot" framing; v1 ships chat + BigQuery logs, v1.2 layers the DRA tagging pass + analytics chat. **R1 reframes** from "which framework" to JB/AR sign-off on: (i) the Boldkast DRA map (the brief proposes 4 DRAs; AR + JB to confirm/revise), (ii) the four open questions at the end of the brief (appresent priority, activation threshold, MAMCR research track). Lock before the 2026-06-29 holiday freeze so the post-freeze window can build against a fixed DRA contract. ~8 eng-days + ~3–4 JB/AR ped-days. **Lens stack extended 2026-07-11 by [1.1.57 competency-rubrics](v1.1.0-feedback/competency-rubrics.md)** — AR's 29-June sources add Lens C (MAPS problem-solving judge) + Lens D (SAAR inquiry judge), composing with either R1 stack; offline slices un-gated, teacher-facing vocabulary still R1-gated. |
| 2.6 | `multi-subject-curriculum-schema.md` (planned) | v1.1+ / Year-2 — multi-subject expansion | **Roadmap signal (M, 2026-06-29), not committed.** The curriculum corpus + `CurriculumDoc` model are physics-centric (A/B/C `level`, **no `subject` field**). Maths + computer-science teachers may use the app, so the schema needs a first-class **`subject`** dimension (physics / maths / CS / …) across the data model, the `POST /api/curriculum/ingest` endpoint, and the activity-builder Materials-picker filter — plus per-subject level taxonomies (maths/CS stratify differently than physics A/B/C). **Interim stop-gap:** the 29-June free-to-use mathematicus.dk maths textbooks are tagged by `topic` (`matematik – …`) since no `subject` field exists; migrate when this lands. See [v1.1.0-feedback/june-29-feedback.md](v1.1.0-feedback/june-29-feedback.md). Decide priority after the physics pilot. |
| 2.7 | `code-ide-workbench-type.md` (planned) | B / v1.1+ — **Type 6** under [1.J expanded-workbench-types](v1.0.0-pilot/expanded-workbench-types.md) | **Roadmap signal (M, 2026-06-29), not committed.** "Read an IDE environment in the workbench" — a new workbench **Type 6 (Code/IDE)** that exposes a student's editor/file/run state to the tutor (for computer-science + computational-physics activities), the same way Types 3–5 expose sensor / video / lab-notebook state. Needs a `postMessage` contract for editor+run state + the trust-card and snapshot-push wiring (`workbench-element-builder` skill). Pairs with 2.6 (CS as a subject). Year-2 / post-pilot; scope after the pilot. |
| 2.8 | `rag-improvements.md` (planned) — **RAG improvements umbrella** | v1.1+ / Year-2 — incremental, "we can always improve" | **Roadmap signal (M, 2026-06-30), not committed.** A running list of retrieval-quality bets: **(i) parsing backend** — prefer `docparse --pdf-backend liteparse`/`ai` over default pdftotext for structured/figure-heavy docs (done for the textbooks); **(ii) figure/multimodal retrieval** (below); **(iii) per-chunk image metadata** (below). Sub-item (ii)/(iii): textbook **figures** (diagrams, graphs) aren't retrievable today — the managed **Vertex RAG Engine corpus is text-only** (default `text-embedding-005`, no image ingest — *verified* against Google docs 2026-06-30) and **auto-chunks** (no per-chunk metadata control). Two capabilities both require **owning the vector store** (Vertex AI **Vector Search**, or the deferred [1.3 pgvector] Year-2 store): **(a) multimodal image embeddings** — embed figures via `multimodalembedding@001` or **Gemini Embedding 2** (natively multimodal, unified space) for cross-modal retrieval (text query → figure); **(b) per-chunk image metadata** — attach a figure ref to each chunk so a matched chunk **pulls its image for display** [M's pattern]. A **doc-level approximation of (b) works on the CURRENT corpus** (retrieval already returns `source_display_name` → map chunk → its doc → that doc's stored images); exact per-chunk needs the self-managed store. Dovetails with the **self-hosting / local-Gemma** tier (ADR-003) — a local multimodal embedder avoids per-image Vertex cost. Interim today: **figure captions are already grounded** (they're in the prose); richer visual descriptions need a small custom Gemini render-and-describe pass (docparse `--describe` extracts 0 for these vector-derived figures). |
| 2.9 | [aipla/post-pilot/knowledge-graph-and-student-matching.md](post-pilot/knowledge-graph-and-student-matching.md) | **C** / Year-2 — design captured, not committed | **Roadmap signal (M, 2026-06-30), not committed.** The 29-June excitement: an **AI-assisted concept/prerequisite graph** of the physics topics, **per-group mastery monitored against it over time** (longitudinal), and **cross-group matching** ("which groups would benefit from meeting in person"). One substrate — a per-group, group-ID-keyed **student model** — feeding three capabilities. Reuses the chat-log pipeline (1.2), the DRA tagging + analytics rubric (2.5), exit tickets (1.1.8), and the curriculum corpus (1.1.25); the deferred Strand-C graph DB ([1.3]) is its scale path. **Execution sketch only**; the knowledge model + research questions (does matching help? social-network evolution?) are **Strand C** in the scoping site. Gated on pilot data + JB/AR Strand-C scoping + a DPIA for longitudinal student modelling. Extends the 23-June C3 "AI generates the learning map" thread. **Near-term slice (M, 2026-06-30) — a "living concept map":** a teacher-authored `conceptMap` activity element (co-pilot-proposable) + the tutor **checking nodes off against it in-session** as the student works. This slice is **pre-pilot-buildable** (per-session, no longitudinal pipeline / cross-group anything) and is the pull-forward candidate — **scope/UX-coherence-gate call pending M.** Full execution spec (phased M0–M3 + eval): [v1.1.0-feedback/living-concept-map.md](v1.1.0-feedback/living-concept-map.md). |
| 2.10 | [aipla/post-pilot/student-generated-apps-tldraw.md](post-pilot/student-generated-apps-tldraw.md) | **Engine/scene substrate v1.1-buildable** / AI on-ramp **v2** — investigation | **Investigation (M, 2026-07-20).** Evolved from *"tldraw for student-made physics apps — MCP App or embedded?"* to the real goal: **a safe + powerful way for students/teachers to *create* sims as they investigate, beyond what we hand-roll.** **Answer: the engine/scene split** — we hand-roll one trusted [matter.js](https://github.com/liabru/matter-js)-based *engine* (MIT, 83 KB, fits ADR-013's cap, Boldkast pipeline) that runs user/AI-authored *scenes* (validated JSON **data, never code**). Safe by construction (no arbitrary code; the vetted engine — not the author — computes dynamics, so *wrong physics is impossible*) **and** powerful (any scene the engine expresses ≫ any fixed library; grows via a domain ladder: rigid bodies → springs → fields → circuits). Three authoring on-ramps (GUI / NL→scene / remix) → one schema. This is the [Algodoo](https://www.algodoo.com/)/[Falstad](https://www.falstad.com/) architecture, self-hostable, reusing the artefact + authoring + tutor-wire plumbing. **tldraw ruled out for the runtime** (proprietary — watermark/business licence + prod key; **corrects the "also MIT" error in 1.J**); at most a Step-3 authoring GUI. Build path: Step 1 one matter.js sim (ship now) → Step 2 externalise scene schema → Step 3 AI/GUI on-ramp (only this gates a JB/AR *plausibility* spike; safety is already handled). Student analog of 2.4 (teacher-artefact-authoring) — likely one scene object, two audiences. |
