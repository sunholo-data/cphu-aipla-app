...
| Auth | [ADR-001](../_scoping-snapshot/architecture.qmd#adr-001-student-identity-no-auth-anonymous-group-ids) anonymous group IDs (no auth standard exists for "deliberately no identity") | Inherited template provider |
...
4. **Use Danish stx vocabulary** where the seeded problem uses Danish terms; English fallback for AR-trial alignment with [examples.qmd](https://aipla.ku.dk/project/activities).
...
**New GCP project:** `aipla-dev-2026` in `europe-north1` (Finland) per [ADR-007](../_scoping-snapshot/architecture.qmd#adr-007-cloud-region).
...
- [ ] Write `resources/seed-problem.md` (use AR's projectile-motion problem from [examples.qmd](https://aipla.ku.dk/project/activities) as fallback; see Open Question 2)
...
- **No PII collected.** Anonymous group ID is the only identifier ([ADR-001](../_scoping-snapshot/architecture.qmd#adr-001-student-identity-no-auth-anonymous-group-ids)).
...
1. **Seeded problem set source.** Use AR's projectile-motion problem from [examples.qmd](https://aipla.ku.dk/project/activities) as the v0.1 seed? Or ask AR for a fresh one (turnaround risk vs. authenticity gain)? **Recommend: use the projectile-motion example for v0.1**, ask AR for fresh content during the buffer week.
...
- Scoping site (external, design source-of-truth):
  - [architecture.qmd](../_scoping-snapshot/architecture.qmd) — ADRs 001 (anonymous group IDs), 002 (template adoption), 003 (model tiers), 006 (GCP EU), 007 (europe-north1), 008 (model router), 015 (multi-surface UI — v0.1 uses chat surface only)
  - [strands.qmd](https://aipla.ku.dk/project/workstreams) — v0.1 skill commitment: "Problem-set hints (v0.1 Jutland demo + v1)"
  - [timeline.qmd](https://aipla.ku.dk/project/progress) — phase 0.5 "Jutland demo (v0.1)"
  - [examples.qmd](https://aipla.ku.dk/project/activities) — AR's projectile-motion example, fallback seed problem set
