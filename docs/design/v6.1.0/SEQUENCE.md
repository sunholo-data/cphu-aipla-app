...
**Status as of 2026-05-16:** Items 1.0–1.3, 1.5, **1.6 channels ✅ (all 5 milestones shipped 2026-05-16)**, **1.6a discord-channel ✅**, 1.7, 1.8–1.11, 1.13–1.15, 1.17, 1.18, 1.20–1.25 implemented. Remaining: 1.4 local-dev-cli, 1.12 document-data-layer (subcollection migration), 1.19 a2ui-workshop-demo, 1.26 mcp-app-render-ux (snapshot history + pinned panel — workshop polish), 1.27 chat-history-tool-call-hydration (post-workshop).
...
| 1.6 | [channels.md](channels.md) ✅ | **P1** | **3.5d → 1 calendar day** | agent-factory (✅), auth (✅) | **Shipped 2026-05-16 — all 5 milestones via parallel Task sub-agents.** `BaseChannel` framework + 4 adapters (Discord, Email, Telegram, WhatsApp) + CLI demo + Cloud Run TF module + adapter howto. Net 196 new tests (871 → 1067 backend). All evaluator rounds PASS (M1=95, M2=92, M3=94, M4=90, M5=96 / 100). Framework gaps surfaced by M2/M3 round-1 evals (shared `_dispatch_inbound`, `inbound.metadata` → `OutboundMessage.metadata` forward) closed in commit 6c55b43. Adding a new channel = ~4h documented at `docs/integrations/channels-adapter-howto.md`. Sprint plan at [channels-sprint.md](channels-sprint.md). |
| 1.6a | [discord-channel.md](discord-channel.md) ✅ | **P1** | 7h (M2 of 1.6) | channels framework (1.6 M1 ✅) | **Shipped 2026-05-16** (commit fa15281, merged 51ea365). Discord adapter as `BaseChannel` subclass — 656 LOC + 322 Terraform (cloud-run-channel module passes `terraform validate`) + 39 tests. Lifted mechanics from `/Users/mark/dev/ml/edmonbrain/discord-bot/bot.py` (472 LOC, Flask-era). Gateway path (mentions/DM) routes through `BaseChannel._dispatch_inbound` after the M2/M3 cleanup. Cloud Run `min-instances=1` required (cold start kills gateway). Sprint evaluator round 1: PASS 92/100. |
| 1.7 | [mcp-app-integrations.md](mcp-app-integrations.md) | P1 | 3–4d | streaming (✅), tools-porting (✅) | Tools-porting done — McpToolset pattern proven. Can start now. |
...
| 1.10 | [document-ui.md](document-ui.md) ✅ (rendering) | P1 | 3d | file-browser (1.2 ✅), document-to-ai-pipeline (1.9 ✅) | Document workspace shipped. `DocumentViewer`, `DocumentPanel`, split-pane render via custom `BlocksRenderer` (not A2UIViewer — see [implemented/document-rendering-decision.md](implemented/document-rendering-decision.md)). Clicking a doc tab shows the parsed document alongside chat. **SkillsBar carved out → 1.11.** |
...
| 1.13 | [chat-history-fixes.md](chat-history-fixes.md) | **P0** | 1d | chat-history (✅ v6.0.0), chat-session-history (1.8 ✅) | Hardening sprint. Fixes 4 live failure modes (refresh 404, mid-stream flicker, doc-linked history, title rename) with paired regression tests. No new endpoints, no schema changes. **M1+M2 shipped 2026-04-27 (commits d83b1e4, 6411d1f). M3 E2E never ran — three deeper bugs surfaced; folded into 1.14.** |
| 1.14 | [chat-history-deep-fixes.md](chat-history-deep-fixes.md) | **P0** | 1d (shipped 2026-04-27) | chat-history-fixes (1.13 ✅ M1+M2) | Diagnostic-first follow-up. F1 agent-identity guard shipped (commit ed1e9ba). **Exposed deeper backend layer** — see 1.15. |
...
| Split pane layout (no resize handle yet) | [document-ui.md](document-ui.md) | v6.1.0 (1.10) ✅ done |
